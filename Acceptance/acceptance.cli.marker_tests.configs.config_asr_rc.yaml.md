...
nirvana:  
    ...
    quota: voice-core  
    # Specifies the quota for Nirvana workflow execution as `voice-core`.  
...
yt:  
    ...
    token: robot-acoustic-team_yt_token  
    # Specifies the YT token for authentication.  
    # The token is associated with the `robot-acoustic-team` account.
    ...
    pool: voice  
    # Specifies the YT pool as `voice`.  
    # This determines the resource pool for YT operations.

    mr_account: voice  
    # Specifies the MapReduce (MR) quota account as `voice`.  
...
yql:  
    ...
    token: robot-acoustic-team_yql_token  
...
arcanum:  
    ...
    token: robot-acoustic-team-arcanum-token  
...
alice:  
    ...
    uniproxy_url: 'wss://beta.uniproxy.alice.yandex.net/alice-uniproxy-asr-rm/uni.ws'  
