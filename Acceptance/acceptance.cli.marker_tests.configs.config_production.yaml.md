...
nirvana:  
    ...
    quota: voice-analytics  
    # Specifies the quota for Nirvana workflow execution as `voice-analytics`.  
    # This determines the resource allocation for the workflow.

    hitman_guid: 41f39437-f9fc-4ecf-b0de-7c8729a506b3  
    # Specifies the GUID of the Nirvana workflow (Hitman).  
    # This GUID uniquely identifies the workflow.
...
yt:  
    ...
    pool: voice  
    # Specifies the YT pool for resource allocation as `voice`.  
    # This determines the resource pool for YT operations.

    mr_account: alice  
    # Specifies the MapReduce (MR) quota account as `alice`.  
...
soy:  
    ...
    token: robot-alicesoykpi-soy-token  
    # Specifies the SOY token for authentication.  
    # The token is associated with the `robot-alicesoykpi` account.
...
alice:  
    ...
    scraper_timeout: '4h'  
    # Specifies the timeout for the scraper as `4h` (4 hours).  
    # This determines how long the scraper will wait before timing out.

    scraper_pool: alice-kpi  
    # Specifies the pool for the scraper as `alice-kpi`.  
